                    ...
                    ...)
{
  ...
  if (random.distribution=="np"){
      ...
      if(length(fit$coef)==1){followmass<-matrix(Eta[(1:k)*N],1,k)-offset[(1:k)*N]} else {followmass<-matrix(fit$coef[1]+sz[(1:k)*N],1,k)}
  } else {
 ...
 # Initialize for EM loop
  ...
  ##########Start of EM ##########
      ...
      # up to version 0.30: 
      # p <- as.vector(tapply(w,kindex,mean))
  ...
  if (random.distribution=="np") {
      ...
      #Residuals y-Xb, for use in plots
      if (family$family=="binomial"){
