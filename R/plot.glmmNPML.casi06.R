...
function(x,plot.opt=15, noformat= FALSE, ...){
  ...
  if (plot.opt %%4 %in% c(2,3) ){#  EM Trajectories
      R<- x$Misc$res; followmass<-x$Misc$EMTraj[-1,]; ylim<- c(min(followmass)-0.25, max(followmass)+0.25)
      plot(1:x$EMiter,followmass[,1],col=1,type='l',ylim=ylim,ylab='mass points',xlab='EM iterations')
      for (i in 1:k){   
          lines(1:x$EMiter, followmass[,i],col=i)
